...
| 缓存有效期 | 缓存数据的保存时长（0 = 禁用缓存即每次访问均重新搜索） | 7 天 |
...
> **注意**：修改高级设置可能影响脚本性能，或导致频繁的 429 限流事件。默认设置已在安全前提下实现最佳性能。
...
A: 首次需要遍历所有好友检查评测，实测229个好友需要加载1分钟左右。之后会使用缓存，秒加载。
...
> **Note**: Modifying advanced settings may affect script performance or cause frequent 429 rate limit errors. Default settings are optimized for best performance while staying safe.
